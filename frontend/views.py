    ...
    if request.user.is_authenticated:
        ...
        if request.user.profile.stage == 'profile_type_selection':
            ...
            if request.user.profile.user_type == 'developer':
                try:
                    student = Student.objects.using('quiz').get(user=request.user.id)
                    passedquizz = TakenQuiz.objects.using('quiz').filter(score__gte=50).filter(student_id=student)
                    ...
                    obj = Student(user=request.user.id)
                    obj.save(using='quiz')
...
def passedquizzes(request):
    taken = TakenQuiz.objects.using('quiz').all()
    students= User.objects.filter(profile__user_type='developer')

    return render(request, 'frontend/recruiter/takenquizzes.html',{'taken':taken,'students':students})
def failedquizzes(request):
    taken = TakenQuiz.objects.using('quiz').all()
    students= User.objects.filter(profile__user_type='developer')
    return render(request, 'frontend/recruiter/failed.html',{'taken':taken,'students':students})
...
def calltoapply(request):
    ...
    takenquizzes = TakenQuiz.objects.using('quiz').filter(student_id=student)
    allquizid=[]
    ...
    for unique in uniquesubjects:
        allstudentquizzes = TakenQuiz.objects.using('quiz').filter(quiz__subject_id=unique.id).filter(student_id=student)
...
def portfolio(request):
    ...
    student = Student.objects.using('quiz').get(user=request.user.id)
    verified_skills = TakenQuiz.objects.using('quiz').filter(student=student).filter(score__gte=50).all()
    skill = []
...
def analytics(request):
    passedtests = TakenQuiz.objects.using('quiz').filter(score__gte=50).annotate(month=TruncMonth('date')).values(
        'month').annotate(
    ...
    failedtests= TakenQuiz.objects.using('quiz').filter(score__lt=50).annotate(month=TruncMonth('date')).values('month').annotate(
        total=Count('student_id'))
    ...
    alltested = TakenQuiz.objects.using('quiz').annotate(month=TruncMonth('date')).values('month').annotate(
        total=Count('student_id'))
